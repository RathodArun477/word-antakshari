...
def handle_use_skip(data=None):
    ...
    with room_registry.room_session(room_code) as room:
        ...
        if not room.is_current_turn(player.player_id):
            socketio.emit("error", {"code": "OUT_OF_TURN", "message": "Not your turn"}, room=request.sid)
            return
        ...
        room.previous_word = None  # skipped turn -> nothing to guess next turn
        room.previous_turn_player_id = None

        socketio.emit("skip_used", {"player_id": player.player_id}, room=room.room_code)
